...
If runtime activation fails after the source fast-forward, upgrade attempts to restore the old dependencies and build. If recovery also fails, it retains the staging directory and backups, reports their paths and prints guarded restore commands. Resolve the filesystem error and recover the missing directories before running `codex-hud-sync`.
...
The default view uses the terminal foreground with a cyan accent, bold project names, and quieter labels, with completed tool history collapsed by default. Yellow marks permissions, approval waits or capacity pressure; red marks errors or critical capacity. Overview columns align with whitespace. Press `d` for environment inventory, Git file counts, token breakdown and session identity; `t` changes only tool detail. A three-second `Details: compact/full` confirmation appears in the header even when both modes show the same rows. Titles in both views shorten absolute paths to their project/file names and use more space in wide windows; useful task text takes priority over uptime and long branches. Narrow panes shed the gauge and equivalent token count before the remaining percentage or bold compact count, and the header reserves space for `[view]`.
...
Terminal resizes preserve that manual height within the available window. Layout
updates keep the selected pane. New sessions initially focus the Codex pane.
The wrapper also inherits the existing tmux server's `escape-time` setting.
...
New sessions capture `CODEX_HOME` and `CODEX_SESSIONS_PATH` for both Codex and the HUD, even with an existing tmux server. Reload keeps those data roots aligned with the running Codex, while refreshing HUD display settings from the caller. Empty/unset display settings explicitly clear inherited server values, including `NO_COLOR`.
...
| Date | Change |
|------|--------|
| 2026-09-24 | Managed daemon: a TUI's startup probe connection (`experimentalFeature/list` only) no longer takes its pane's pairing, which left new panes unbound for 15 minutes; the daemon connection now decides before the pane's own log rows, and the agents overview's rows no longer bind a pane to another pane's session; HUDs hand the daemon connection ledger on through a state file, so a reloaded HUD keeps a long session whose early log rows Codex has pruned (it showed the thread the pane had left, or nothing); a TUI running its own app-server beside a daemon binds by its own rows again and stays out of the daemon's pairing |
| 2026-09-23 | codex 0.156 managed daemon (`features.daemon_auto_start`): each pane binds through its own daemon connection (`app_server.connection_id`, paired by the TUI's connect time, else by launch order) instead of the pane process tree, which held no thread for most panes and every pane's threads for the one that launched the daemon; daemon clients never fall back to shell snapshots or the cwd guess |
