...
| Date | Change |
|------|--------|
| 2026-09-24 | Managed daemon: a TUI's startup probe connection (`experimentalFeature/list` only) no longer takes its pane's pairing, which left new panes unbound for 15 minutes; the daemon connection now decides before the pane's own log rows, and the agents overview's rows no longer bind a pane to another pane's session; HUDs hand the daemon connection ledger on through a state file, so a reloaded HUD keeps a long session whose early log rows Codex has pruned (it showed the thread the pane had left, or nothing) |
