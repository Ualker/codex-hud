...
|------------|---------------------------------------------|
| 2026-06-05 | 支持 detach/终端关闭后保留 tmux 会话并默认重连 |
...
- 修复时间：2026-06-05
- 变更说明：不带 Codex CLI 参数启动时默认重连同目录最新 HUD session；wrapper 退出只清理未完成启动的 session，不再因 detach 或终端关闭杀掉已就绪 tmux session。
- 验证方式：通过 fake tmux 集成测试覆盖默认重连、带参数默认新建、显式复用、强制新建，以及 attach client 退出后不调用 `kill-session`。
